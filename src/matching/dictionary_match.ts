 ...
 */
export function dictionary_match(password: string, _ranked_dictionaries = RANKED_DICTIONARIES): IDictionaryMatch[] {
 ...
 */
export function relevant_l33t_subtable(password: string, table: Record<string, string[]>): Record<string, string[]> {
	const password_chars = new Set(password.split(""));
	const subtable: Record<string, string[]> = {};

	for (const letter in table) {
		const relevant_subs = table[letter].filter((sub) => password_chars.has(sub));
 ...
 */
export function enumerate_l33t_subs(table: Record<string, string[]>): Record<string, string>[] {
...
export function l33t_match(
	...
	const matches: IDictionaryMatch[] = [];
	for (const sub of enumerate_l33t_subs(relevant_l33t_subtable(password, _l33t_table))) {
		if (empty(sub)) {
			break; // corner case: password has no relevant subs.
		}
		const subbed_password = translate(password, sub);
		for (const match of dictionary_match(subbed_password, _ranked_dictionaries)) {

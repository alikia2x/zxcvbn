...
export function spatial_match(
	...
	return sorted(
		([] as ISpatialMatch[]).concat(
			...Object.keys(_graphs).map((graph_name) => spatial_match_helper(password, _graphs[graph_name], graph_name))
		)
	);
}
...
export function spatial_match_helper(
	...
	while (i < password.length - 1) {
		...
		let turns = 0;
		if (["qwerty", "dvorak"].includes(graph_name) && SHIFTED_RX.exec(password.charAt(i))) {

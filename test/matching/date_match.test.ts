	...
	describe("date_match", () => {
		...
		it("matches dates with varied element order", () => {
			...
			for (const order of ["mdy", "dmy", "ymd", "ydm"]) {
				...
				const m = 8;
				const y = 88;
				const password = order.replace("y", y.toString()).replace("m", m.toString()).replace("d", d.toString());
		...
		it("matches embedded dates", () => {
			...
			for (const { password, i, j } of generatePasswords(pattern, prefixes, suffixes)) {
				const expected: IDateMatch[] = [
					{
						i,
						j,

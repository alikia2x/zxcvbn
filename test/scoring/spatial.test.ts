...
describe("scoring", () => {
	describe("spatial_guesses", () => {
		it("guesses starts * degree * (len-1) when there are no turns or shifts", () => {
			...
			const expected = KEYBOARD_STARTING_POSITIONS * KEYBOARD_AVERAGE_DEGREE * (match.token.length - 1);
		...
		it("doubles the guesses if everything is shifted", () => {
			...
			const expected = KEYBOARD_STARTING_POSITIONS * KEYBOARD_AVERAGE_DEGREE * (match.token.length - 1) * 2;

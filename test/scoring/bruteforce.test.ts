...
describe("scoring", () => {
	describe("most_guessable_match_sequence", () => {
		...
		it("chooses match with the fewest guesses given two matches of the same span", () => {
			...
			const result1 = most_guessable_match_sequence(password, [worseMatch, bestMatch], true);
			expect(result1.sequence).toMatchObject([worseMatch]);

			const result2 = most_guessable_match_sequence(password, [bestMatch, worseMatch], true);
		...
		it("chooses m0 when it covers m1 and m2, if m0 < m1 * m2 * 2!", () => {
			...
			const result = most_guessable_match_sequence(password, [m0, m1, m2], true);

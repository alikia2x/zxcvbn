	...
	describe("repeat_guesses", () => {
		it("calculates the right number of guesses", () => {
			for (const [token, base_token, repeat_count] of [
				...
				const base_guesses = most_guessable_match_sequence(base_token, omnimatch(base_token)).guesses;
		...
		it("is delegated to by estimate_guesses", () => {
			...
			const base_guesses = most_guessable_match_sequence(baseToken, omnimatch(baseToken)).guesses;
			const match: IRepeatMatch = {
				token,
